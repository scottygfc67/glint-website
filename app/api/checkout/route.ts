...
// Extract numeric variant ID from a Shopify GID if needed
function toVariantNumericId(id: string): string {
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get variant ID from environment variables (should already be numeric)
    ...
    const checkoutUrl = `https://${domain}/cart/${toVariantNumericId(String(variantId))}:${qty}`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Items in cart use variant GIDs; convert to numeric IDs for /cart URL
    const cartItems = items
      .map((item: { variantId: string; quantity: number }) => {
        const id = toVariantNumericId(String(item.variantId));
